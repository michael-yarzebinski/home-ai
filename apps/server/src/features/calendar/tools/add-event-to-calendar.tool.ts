...
const AddEventToCalendarToolSchema = z.object({
  ...
  endDateTime: z
    ...
    .describe(
      "End date and time of the event.  If the user does not provide an end time, use the start time plus 1 hour.",
    ),
  location: z
    .preprocess(ToolParameterUtils.stripQuotes, z.string().optional())

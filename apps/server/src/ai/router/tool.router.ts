...
export class ToolRouter {
  ...
  async dispatch(request: DispatchRequest): Promise<DispatchResult> {
    const matchingTool = this.tools.find(tool => tool.canHandle(request.task.taskName));
    if (!matchingTool) {
      ...
      return {
        success: false,
        message: `No handler for task: ${request.task.taskName}`,
        reply: `Sorry, I don't know how to perform "${request.task.taskName}" yet.`,
      };
    }
    ...
    try {
      const requestData: any = {
        task_name: request.task.taskName,
        requester_user_id: request.user.userId,
        executor_user_id: request.user.userId,

...
import { WeatherRequest, WeatherTimePeriod } from '@home-ai/shared/domain/weather/weather-request';
...
export class WeatherService {
  ...
  async getWeather(zipCode: string, weatherRequest: WeatherRequest): Promise<any> {
    ...
    if (weatherRequest.timePeriod === WeatherTimePeriod.CURRENT) {
      params.current = 'temperature_2m,relative_humidity_2m,apparent_temperature,is_day,precipitation,rain,showers,snowfall,weather_code,wind_speed_10m,wind_direction_10m';
      params.forecast_days = weatherRequest.days ?? 1;
    }
    else if (weatherRequest.timePeriod === WeatherTimePeriod.HOURLY) {
      params.hourly = 'temperature_2m,relative_humidity_2m,apparent_temperature,precipitation_probability,precipitation,weather_code,wind_speed_10m,is_day';
      params.forecast_days = weatherRequest.days ?? 1;
    }
    else if (weatherRequest.timePeriod === WeatherTimePeriod.DAILY) {
      params.daily = 'weather_code,temperature_2m_max,temperature_2m_min,apparent_temperature_max,apparent_temperature_min,sunrise,sunset,uv_index_max,precipitation_sum';
    }

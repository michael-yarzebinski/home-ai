...
const GetAutomationRulesSchema = z.object({
  ...
  entityId: z
    ...
    .describe(
      "Optional: Filter by a specific entityId or deviceId within the trigger config",
    ),
});

@Tool()
@Injectable()
export class GetAutomationRulesTool extends ToolHandler<
  typeof GetAutomationRulesSchema
> {
  readonly name = "get-automation-rules";
  readonly description =
    "Retrieve active automation rules filtered by trigger type and optionally a specific entity/device ID.";
  ...
  async execute(
    params: z.infer<typeof GetAutomationRulesSchema>,

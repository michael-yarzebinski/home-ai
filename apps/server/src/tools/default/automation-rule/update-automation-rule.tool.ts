...
const UpdateAutomationRuleSchema = z.object({
  ...
  active: z.preprocess(ToolParameterUtils.toBooleanValue, z.boolean().optional()),
  name: z
  ...
  actions: z.preprocess(ToolParameterUtils.toUnknownArray, z.array(z.any()).optional())
});

@Tool()
@Injectable()
export class UpdateAutomationRuleTool extends ToolHandler<typeof UpdateAutomationRuleSchema> {
  readonly name = 'update-automation-rule';
  readonly description = 'Modify an existing automation rule (e.g., enable/disable, change schedule).';

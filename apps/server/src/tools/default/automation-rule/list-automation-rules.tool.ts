...
@Tool()
@Injectable()
export class ListAutomationRulesTool extends ToolHandler<typeof ListAutomationRulesSchema> {
  readonly name = 'list-automation-rules';
  readonly description = 'Retrieve all active automation rules for the current user.';
  ...
  async execute(_params: any, context: ToolContext) {
    ...
    return {
      count: rules.length,
      rules: rules.map(r => ({ id: r.id, name: r.name, active: r.active, trigger: r.trigger }))
    };
  }
}
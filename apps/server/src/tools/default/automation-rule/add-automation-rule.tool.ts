...
import { z } from 'zod';
import { TriggerType, ActionType, TriggerConfig } from '@home-ai/shared/domain/automation-rule/automation-rule';
...
const ActionSchema = z.object({
    ...
    instruction: z.preprocess(ToolParameterUtils.stripQuotes, z.string()).describe('Natural language goal for the action'),
    metadata: z.preprocess(toRecord, z.record(z.string(), z.any()).optional())
});

const AddAutomationRuleSchema = z.object({
    name: z.preprocess(ToolParameterUtils.stripQuotes, z.string()).describe('Short descriptive name for the rule'),
    description: z
    ...
    cooldownMinutes: z
        .preprocess(ToolParameterUtils.toNumberValue, z.number().int().min(0).optional())
        .default(60)
        .describe('Minimum minutes between rule executions. Optional; defaults to 60.')
});

@Tool()
@Injectable()
export class AddAutomationRuleTool extends ToolHandler<typeof AddAutomationRuleSchema> {
    readonly name = 'add-automation-rule';
    readonly description = 'Create a new automation rule that reacts to home events or schedules.  You may need to call the list-devices tool to get the deviceId.  You should NOT discover.';
    readonly parameters = AddAutomationRuleSchema;

    constructor(private readonly automationStore: AutomationRuleStore) { super(); }

    async execute(params: z.infer<typeof AddAutomationRuleSchema>, context: ToolContext) {
        const trigger = buildTriggerConfig(params.triggerType, params.triggerParams);
        ...
        return { success: true, ruleId: rule.id, message: `✅ Automation "${params.name}" created.` };
    }
}

const getRequiredString = (params: Record<string, any>, key: string): string => {
    ...
    switch (triggerType) {
        ...
        case TriggerType.SYSTEM: {
            ...
            return {
                ...
                ...(typeof intent === 'string' && intent.trim().length > 0 ? { intent: intent.trim() } : {}),
            };
        }
        default:

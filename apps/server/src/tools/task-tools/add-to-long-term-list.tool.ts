...
export const AddToLongTermListParamsSchema = `
{
  "type": "object",
  "properties": {
    "items": {
      "type": "array",
      "description": "List of checklist items to add to the long-term list",
      "items": { "type": "string", "description": "A long-term list entry" }
    }
  },
  "required": ["items"]
}
...
@Injectable()
@RegisterTask(TaskName.AddToLongTermList)
export class AddToLongTermListTool extends TaskHandlerBase {
  ...
  readonly metadata: TaskHandlerMetadata = {
    ...
    parameters: AddToLongTermListParams,
    parametersSchema: AddToLongTermListParamsSchema,
  ...
  async execute(request: TaskHandlerContext): Promise<TaskHandlerResult> {
    ...
    if (items.length === 0) {
      return {
        status:  TaskHandlerStatus.CLARIFICATION_NEEDED, reply: 'No items provided to add to long term list.',
      };
    }
    ...
    if (stderr) {
      ...
      return { status:  TaskHandlerStatus.ERROR, error: 'Failed to update long term list.' };
    }

    return {
      status: TaskHandlerStatus.SUCCESS, reply: stdout.trim() || `Added ${items.length} item(s) to Long Term List.`,
    };

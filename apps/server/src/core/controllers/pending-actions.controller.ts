import {
  ...
  HttpCode, HttpStatus, NotFoundException, Param, Post,
} from '@nestjs/common';
import { z } from 'zod';
...
export class PendingActionsController {
  ...
  async approve(@Param('id') id: string, @CurrentUser() user: AuthUser) {
    ...
    if (!canApprove) throw new ForbiddenException('Insufficient role to approve actions');
  ...
  async reject(
    ...
    if (!canReject) throw new ForbiddenException('Insufficient role to reject actions');

    return this.pendingActionStore.update(id, { status: 'rejected', reason: dto.reason });
  }
}

...
import {
  InsertablePendingActionSchema, UpdatablePendingActionSchema,
  type InsertablePendingAction, type UpdatablePendingAction,
...
export class PendingActionsAdminController {
  ...
  create(@Body(new ZodValidationPipe(InsertablePendingActionSchema)) dto: InsertablePendingAction) {
    return this.pendingActionStore.create(dto);
  }

  @Put(':id')
  update(@Param('id') id: string, @Body(new ZodValidationPipe(UpdatablePendingActionSchema)) dto: UpdatablePendingAction) {

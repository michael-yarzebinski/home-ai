...
import {
  InsertableUserApiSchema, UpdatableUserApiSchema,
  type InsertableUserApi, type UpdatableUserApi,
} from '@home-ai/shared/domain/user/user';

// Admin create: API-safe fields + a plaintext accessCode that gets hashed before storage.
const AdminCreateUserSchema: ReturnType<typeof InsertableUserApiSchema.extend<{ accessCode: z.ZodString }>> =
...
export class UsersAdminController {
  ...
  async create(@Body(new ZodValidationPipe(AdminCreateUserSchema)) dto: AdminCreateUserDto) {
  ...
  @Put(':id')
  update(@Param('id') id: string, @Body(new ZodValidationPipe(UpdatableUserApiSchema)) dto: UpdatableUserApi) {

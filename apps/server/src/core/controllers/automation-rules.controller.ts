...
export class AutomationRulesController {
  ...
  search(
    ...
    return this.automationRuleStore.search(dto, user);
  }
  ...
  create(
    @Body(new ZodValidationPipe(InsertableAutomationRuleSchema)) dto: InsertableAutomationRule,
    @CurrentUser() user: AuthUser,
  ) {
  ...
  update(
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdatableAutomationRuleSchema)) dto: UpdatableAutomationRule,
    @CurrentUser() user: AuthUser,
  ) {
  ...
  async restore(@Param('id') id: string, @CurrentUser() user: AuthUser) {
    await this.automationRuleStore.restore(id, user);
    return this.automationRuleStore.getById(id, true, user);
  }
}

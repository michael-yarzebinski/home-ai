...
import { type DashboardPeriod, type DashboardResult, type TimeBucket } from '@home-ai/shared/domain/admin/dashboard/dashboard';

// Truncation unit drives how many data points appear in the chart.
const PERIOD_CONFIG: Record<DashboardPeriod, { intervalSql: string; truncUnit: string; periodMs: number }> = {
  '1h':  { intervalSql: '1 hour',   truncUnit: 'minute', periodMs: 60 * 60 * 1000 },
  '24h': { intervalSql: '24 hours', truncUnit: 'hour',   periodMs: 24 * 60 * 60 * 1000 },
  '7d':  { intervalSql: '7 days',   truncUnit: 'day',    periodMs: 7 * 24 * 60 * 60 * 1000 },
  '30d': { intervalSql: '30 days',  truncUnit: 'day',    periodMs: 30 * 24 * 60 * 60 * 1000 },
  '90d': { intervalSql: '90 days',  truncUnit: 'week',   periodMs: 90 * 24 * 60 * 60 * 1000 },
};
...
export class DashboardService {
  ...
  async get(period: DashboardPeriod): Promise<DashboardResult> {
    ...
    const [
      ...
      this.timeBuckets('notification_log', 'created_at', truncUnit, intervalSql),
      this.knex('logs')
        .select('severity')
        .count('* as count')
        .where('created_at', '>=', this.knex.raw(`NOW() - INTERVAL '${intervalSql}'`))
      ...
      this.knex('logs')
        ...
        .limit(10) as Promise<Array<{ id: string; message: string; createdAt: Date; metadata: unknown }>>,
      this.knex('notification_queue')
        .select(
          this.knex.raw('COUNT(*)::int AS total'),
          this.knex.raw(`COUNT(*) FILTER (WHERE active = true AND scheduled_for > NOW())::int AS pending`),
        )
    ...
    const mergeBuckets = (
      ...
      key: keyof Pick<TimeBucket, 'logs' | 'aiAudit' | 'audit' | 'notificationLog'>,
    ) => {
      for (const row of rows) {
        const ts = new Date(row.bucket).toISOString();
        const entry = bucketMap.get(ts) ?? { timestamp: ts, aiAudit: 0, audit: 0, logs: 0, notificationLog: 0 };
    ...
    const buckets = Array.from(bucketMap.values()).sort((a, b) => a.timestamp.localeCompare(b.timestamp));

    // Severity summary
    const sev: Record<string, number> = {};
    for (const row of severityRows) sev[row.severity.toLowerCase()] = Number(row.count);
    ...
    const pendingActionsTotal = Object.values(paMap).reduce((sum, n) => sum + n, 0);
  ...
  private timeBuckets(
    ...
    return this.knex(table)
      .select(this.knex.raw(`date_trunc('${truncUnit}', ${tsColumn}) AS bucket`))
      .count('* as count')
